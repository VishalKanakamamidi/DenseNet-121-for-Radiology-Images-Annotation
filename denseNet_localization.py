...
test_list = os.listdir(img_folder_path)
...
for i in range(len(test_list)):
    ...
    opencvimg.append(cv2.imread(os.path.join(img_folder_path, test_list[i]))) 
...
for index in range(len(test_dataset)):
    # print("test_dataset",test_dataset[index])
    input_img = Variable((test_dataset[index]).unsqueeze(0).cuda(), requires_grad=True)
    probs = gcam.forward(input_img)
    raw = cv2.resize(opencvimg[index], (224,224), interpolation = cv2.INTER_AREA)
    # cv2.imwrite("testy.png",np.uint8(test_X*255))

    activate_classes = np.where((probs > thresholds)[0]==True)[0]
    print("Activated Classes",activate_classes) # get the activated class
    for activate_class in activate_classes:
        ...
        hm = cv2.applyColorMap(np.uint8(output * 255.0), cv2.COLORMAP_JET)
        hm = hm.astype(np.float)+raw.astype(np.float)
        hm = hm / hm.max() * 255.0
        if(class_index[activate_class]==dname):
            cv2.imwrite("Out_img//"+str(activate_class)+dt_string+".jpg",np.uint8(hm))
            print(True)
...
with open("bounding_box.txt","w") as f:
    for i in range(len(prediction_dict)):
        ...
        for p in prediction:
            ...
            thickness = 1
            if(arr[0]==dname):
                image = cv2.putText(frame, arr[0], org, font, fontScale,  
                     color, thickness, cv2.LINE_AA, False)

                cv2.rectangle(frame,(x1,y1),(x2,y2),(60,20,220),3)
                print(True)
    cv2.imwrite("Out_img//"+dt_string+fname,frame)